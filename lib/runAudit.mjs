...
function loadErrorMessage(rawMsg) {
  ...
  if (/Target crashed|Page crashed|browser has (?:disconnected|been closed)|Target (?:closed|page).*crashed/i.test(m))
    return "Non sono riuscito a completare la misurazione: la pagina è così pesante da interrompere l'analisi a metà. Riprova tra poco — se succede di nuovo, è di per sé un segnale che il sito carica troppe cose insieme.";
